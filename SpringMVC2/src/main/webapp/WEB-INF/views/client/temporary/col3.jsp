...
/* 	input.addEventListener("change", (event) => {
		  console.log("변함1"+event);
		   const files = changeEvent(event);
		  console.log("변함2"+event);
		   handleUpdate(files);  
		  console.log("변함3"+event);
		}); */
	...
	document.addEventListener("dragenter", (event) => {
		  event.preventDefault();
		   if (event.target.className === "content-area") {
		  ...
		  question = event; 
		   if (event.target.className === "content-area") {
		    ...
		    if(files!=null){
		    	let fileInput= document.getElementsByClassName('fileInputList');
		    	frame.innerHTML += "\n 　";
		    	fileInput[fileInput.length-1].files = files;
		    	addButtonFileInput();
		    	hang(frame);
		    }
		     handleUpdate([...files]);  
